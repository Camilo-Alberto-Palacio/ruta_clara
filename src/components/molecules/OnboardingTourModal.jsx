        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-md overflow-hidden animate-scale-up text-slate-900 dark:text-slate-100 flex flex-col">
                ...
                <div 
                    ...
                    style={{ background: `linear-gradient(135deg, ${slide.color}, #0f172a)` }}
                >
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 flex flex-col gap-4">
                    ...
                    <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/50 text-xs font-semibold text-slate-700 dark:text-slate-200">
                        {slide.tip}
                    </div>
                        ...
                        {SLIDES.map((_, idx) => (
                            <button
                                ...
                                className={`h-2 rounded-full transition-all border-none cursor-pointer ${
                                    currentSlide === idx ? 'w-6 bg-emerald-600' : 'w-2 bg-slate-300 dark:bg-slate-700'
                                }`}
                                title={`Paso ${idx + 1}`}
                            />
                        ))}
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2">
                        {currentSlide > 0 ? (
                            <button
                                type="button"
                                onClick={handlePrev}
                                className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-bold text-xs hover:bg-slate-200 border-none cursor-pointer transition-colors"
                            >
                                Anterior
                            </button>
                        ) : (
